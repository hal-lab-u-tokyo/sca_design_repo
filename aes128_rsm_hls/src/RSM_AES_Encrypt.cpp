...
*    
*    File:          /aes128_rsm_hls/src/RSM_AES_Encrypt.cpp
...
void SubBytes(unsigned char* state, unsigned char rotate)
{
	// #pragma HLS ALLOCATION type=function instances=Sbox limit=4

	for (unsigned char i = 0; i < SIZE; i++) {
		#pragma HLS unroll
		state[i] = masked_sboxes[(i + rotate) % 16][state[i]];
	}
}
...
void RSM_AES128Encrypt(const unsigned char *key, const unsigned char *plaintext, unsigned char *ciphertext, unsigned char rotate)
{
	...
	#pragma HLS array_partition variable=masked_sboxes complete
	#pragma HLS array_partition variable=M complete
	#pragma HLS array_partition variable=MMS complete
	...
	LOOP_ROUND: for (int round = 0; round < 10; round++) {
		...
		// remasking
		if (round != 9) {
			AddRoundKey(state, (unsigned char*)MMS[rotate]);
		} else {
			AddRoundKey(state, (unsigned char*)MS[rotate]);
		}
	}
